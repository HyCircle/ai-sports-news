...
## Phase 7: Refining DB Workflow & Content Management

- **数据库 Schema 演进**:
  - `articles` 表新增 `used INTEGER` 列（1=用于日报，0=被 LLM 忽略/不重要，NULL=待处理）
  - 实现全量标记：无论文章是否被 LLM 聚类选中，只要在该批次中"出现过"，执行完成后统一标记 `report_date`，彻底解决被忽略文章无限重跑的问题。
- **RSS 抓取优化**:
  - `config.py` 新增 `RSS_FETCH_DAYS` 配置（默认 7 天）。
  - `fetcher.py` 恢复并在 Entry 级别应用日期过滤，跳过超过 N 天的旧新闻，防止数据库条目爆炸。
- **稳定性增强**:
  - 完善 `main.py` 的闭环更新机制。
  - DB 操作增加 `try/except` 异常捕获，若写入失败会打印详细提示，文章将保持待处理状态供下次运行。
- **数据迁移**:
  - 已完成一次性数据库迁移，将旧版 `report_date` 的 NULL 项重置，并根据历史记录填充 `used` 列。

## Phase 8: README Update (Refinement)

- **更新 `README.md`**:
  - 增加对 `used` 状态追踪和自动清理被忽略条目的描述。
  - 细化增量抓取策略说明。
